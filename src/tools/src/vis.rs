...
fn vis(input: &Input, out: &Vec<Rect>, show_id: bool) -> String {
...
pub fn visualize(input: &str, output: &str) -> (i64, String) {
    let input = read_input(input);
    let output = read_output(&input, output);
    let score = score(&input, &output);
    let svg = vis(&input, &output, true);
    (score, svg)
}
